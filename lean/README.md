...
3. `grep -rn "sorry\|admit\|native_decide\|sorryAx" Tex2lean/` comes back clean.
...
`Tex2lean/Model/` holds everything the headline theorem asserts, and nothing
else. `Tex2lean/Meta/ModelClosure.lean` machine-checks that boundary on every
build.

**It holds no proofs.** Statements and the vocabulary they are stated in live in
`Model/`; the arguments live in `Tex2lean/Analysis/`, one file per surface file
